...
@pytest.fixture(scope="session")
def account_test_app() -> fastapi.FastAPI:
    """
    A fixture that re-initializes the FastAPI instance for test application.
    """
    testing_app = initialize_account_application()
...
@pytest.fixture(scope="session")
async def async_client(account_test_app: fastapi.FastAPI) -> httpx.AsyncClient:  # type: ignore
    async with asgi_lifespan.LifespanManager(account_test_app):
        async with httpx.AsyncClient(
            app=account_test_app,
        ) as client:
            yield client
