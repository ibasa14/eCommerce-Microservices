...
class InitDB:
    def __init__(self):
        self.db_uri: str = f"{settings.DB_POSTGRES_SCHEMA}://{settings.DB_POSTGRES_USERNAME}:{settings.DB_POSTGRES_PASSWORD}@{settings.DB_POSTGRES_HOST}:{settings.DB_POSTGRES_PORT}/postgres_authentication_testing"
    ...
    def users_table(self):
        data = {
            'id': [2],
            "name": ["test_user"],
            "email": ["test_user@ibc.com"],
            "hashed_password": [password_generator.generate_hashed_password("test_password")],
            "is_active":[True],
            "is_logged_in": [False],
            "role_id":[2]
        }
